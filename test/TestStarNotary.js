...
it('can add the star name and star symbol properly', async() => {
    ...
    let instance = await StarNotary.deployed();
    await instance.createStar('Awesome Star!', tokenId, {from: accounts[0]})
    assert.equal(await instance.symbol(), 'STZ');
    assert.equal(await instance.name(), 'Stellazza');
...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('Star A', tokenId1, {from: accounts[1]});
    assert.equal(await instance.ownerOf.call(tokenId1), accounts[1]);
    await instance.createStar('Star B', tokenId2, {from: accounts[2]});
